...
| 2.1-2.6 | タスクリスト修正 | app.js | fixTaskListNumbering() |
...
| 要件 | 1.1-1.3, 2.1-2.6, 3.1-3.6, 4.1-4.5 |
...
function fixTaskListNumbering(html) {
  ...
  // パターン1: <input><ol><li>...</li></ol> (タイトリスト)
  // パターン2: <p><input></p><ol><li>...</li></ol> (ルーズリスト)
  return html.replace(
...
- fixTaskListNumbering: 各パターンのHTML変換を検証
  - `- [x] 1. テキスト` のケース（タイトリスト）
  - `- [ ] 2. テキスト` のケース（タイトリスト）
  - ルーズリスト（`<p>` タグでラップされたケース）
  - ネストがない通常のタスクリスト
...
### 2026-01: ルーズリスト対応の追加
- **背景**: `- [ ] 1. テキスト` の後に別のリスト項目がある場合、markedが `<p>` タグでinputをラップし、既存の正規表現がマッチしない
- **決定**: 正規表現を拡張して `<p>` タグを許容するパターンに変更
- **理由**: 既存のアーキテクチャを維持しつつ、最小限の変更で問題を解決
