...
5.12 If ttydの起動ポートが使用中の場合, the システムはポートをインクリメントして再試行するものとする（最大30回）
...
5.16 The システムはttyd起動時に `--theme` オプションを使用して、xterm.jsのテーマ（背景色・前景色）をmd-open-browserのダークテーマ配色（背景: `#0d1117`、前景: `#c9d1d9`）と一致させるものとする
...
7.9 When パネルがドラッグリサイズされた場合, the システムはttyd iframe内のターミナルがリサイズに追従するよう、iframeのviewport変更を明示的にトリガーするものとする

7.10 The システムはドラッグ中は150ms間隔でデバウンスしてリサイズ通知を行い、ドラッグ完了時には即時リサイズ通知を行うものとする

7.11 The `.ttyd-frame`の`min-height`は`0`とし、パネルの高さに応じて自由に伸縮可能とするものとする

7.12 When ttydモードで初期化された場合, the パネルはデフォルト高さ`40vh`で表示されるものとする
...
**目的**: ユーザーとして、キャッチアップパネルをマークダウンプレビューの右側に表示したい。それにより、縦方向の表示領域を十分に確保しながらターミナルとプレビューを同時閲覧できる。

### 受け入れ基準

9.1 The `.content` 要素は `display: flex; flex-direction: row` を使用して、マークダウンプレビューとキャッチアップパネルを横並びで配置するものとする（4.1の`flex-direction: column`を置き換え）

9.2 The リサイズハンドルは `width: 6px; cursor: col-resize` で表示し、マークダウンプレビュー領域とキャッチアップパネルの間に垂直バーとして配置されるものとする（7.7の`height: 6px; cursor: row-resize`を置き換え）

9.3 The キャッチアップパネルは `border-left: 1px solid #30363d` でマークダウンプレビューと視覚的に区切られるものとする

9.4 When リサイズハンドルがドラッグされた場合, the システムはX軸の移動量に基づいてキャッチアップパネルの幅を変更するものとする。ハンドルを左方向にドラッグするとパネル幅が増加し、右方向にドラッグすると減少するものとする

9.5 When ttydモードで初期化された場合, the パネルはデフォルト幅 `40vw` で表示されるものとする（7.12の`40vh`高さを置き換え）

9.6 The キャッチアップパネルの幅は最小200px、最大60vwの範囲に制限されるものとする（7.4の高さ制限`150px`~`80vh`を置き換え。80vwではプレビュー領域が使い物にならないためdev-browser検証により60vwに決定）

9.7 When キャッチアップパネルが折りたたみ状態の場合, the パネルはヘッダーのみの幅でコンパクトに表示されるものとする

## 10. ttyd遅延初期化

**目的**: ユーザーとして、ターミナルパネルを使わない場合はttydやグループセッションが起動されないようにしたい。それにより、不要なリソース消費やtmuxペインのズームによる表示への影響を回避できる。

### 受け入れ基準

10.1 The システムはttydプロセス・グループセッション・ズームをアプリケーション起動時に実行せず、`POST /api/tmux/init-ttyd`リクエストを受信した時に初めて実行するものとする

10.2 When `POST /api/tmux/init-ttyd` リクエストを受信した場合, the サーバーはグループセッション作成・ttyd起動を実行し、`{ "ttydUrl": string }` をJSONで返却するものとする
  - Content-Type: `application/json; charset=utf-8`
  - ステータスコード: 200
  - 既に初期化済みの場合は既存のttydUrlを即座に返却（冪等性）

10.3 If `POST /api/tmux/init-ttyd` リクエスト時にtmux連携が無効の場合, the サーバーはステータスコード404を返却するものとする

10.4 If `POST /api/tmux/init-ttyd` リクエスト時にttydコマンドがPATH上に存在しない場合, the サーバーはステータスコード404とエラーメッセージを返却するものとする

10.5 The `GET /api/tmux/status` レスポンスに `ttydAvailable` フィールド（boolean）を追加し、ttydコマンドがPATH上に存在するかどうかを示すものとする

10.6 When tmux連携が有効でttydが利用可能な場合, the キャッチアップパネルの本体部分（`.tmux-panel-body`）はデフォルトで折りたたみ（`collapsed`クラス付与）状態とするものとする

10.7 When キャッチアップパネルが折りたたみ状態から初めて展開された場合で、ttydが利用可能だが未初期化の場合, the システムは`POST /api/tmux/init-ttyd`を呼び出してttydの初期化を行い、初期化完了後にiframeを表示するものとする

10.8 While ttydの初期化中（`POST /api/tmux/init-ttyd`の応答待ち）, the システムはパネル本体にローディング表示（「Starting terminal...」等のテキスト）を行うものとする

10.9 When ttydの初期化に失敗した場合, the システムはプレーンテキスト方式にフォールバックし、テキスト入力・ポーリングUIを表示するものとする

10.10 The ttyd初期化のAPIコールは冪等であり、複数回呼び出しても副作用なく同一のttydUrlを返却するものとする
